(function () {
  ...
  function initialize() {
    try {
      console.log('Tailgunner: Content script initialized');
      setupMessageListener();
    } catch (error) {
      console.error('Tailgunner: Initialization failed:', error.message);
    }
  }

  function log(...args) {
    if (DEBUG) {
      console.log('Tailgunner:', ...args);
    }
  }
  ...
  function getBreakpoint(width) {
    if (!width || typeof width !== 'number' || width < 0) {
      console.warn('Tailgunner: Invalid viewport width:', width);
      return 'unknown';
    }
  ...
  function createPanel() {
    try {
      ...
      el.setAttribute('id', 'tailgunner-panel');
    ...
    `);
      ...
      try {
        ...
        console.error('Tailgunner: Failed to append panel to document:', appendError.message);
        return null;
      }

    } catch (error) {
      console.error('Tailgunner: Failed to create panel:', error.message);
      return null;
    }
  }
  ...
  function updatePanel(panelParts) {
    try {
      ...
      try {
        ...
        console.error('Tailgunner: DOM update failed:', domError.message);
        return false;
      }
    } catch (e) {
      console.error('Tailgunner: Error updating panel:', e.message);
      ...
      if (panel) {
        try {
          ...
          console.warn('Tailgunner: Cleanup after error failed:', cleanupError.message);
        }
      }
      return false;
    }
  ...
  function cleanupPanel() {
    ...
    if (panel && panel.el) {
      try {
        ...
        console.warn('Tailgunner: Error removing panel element:', error.message);
      }
    }
    ...
    try {
      if (panel) {
        ...
        if (panel) {
          ...
          console.error('Tailgunner: ' + errorMsg);
          return { success: false, error: errorMsg };
        }
      }
    } catch (error) {
      console.error('Tailgunner: Error toggling panel:', error.message);
    ...
    try {
      chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        try {
          ...
          console.error('Tailgunner: Error processing message:', error.message);
      ...
      console.error('Tailgunner: Failed to set up message listener:', error.message);
    }
  }
